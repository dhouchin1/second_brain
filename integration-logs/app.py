...
def _db_has(conn, name, kind="table"):
    q = "SELECT name FROM sqlite_master WHERE type=? AND name=?"
    return bool(conn.execute(q, (kind, name)).fetchone())
...
def about(request: Request):
    ...
    info = {
      ...
      "ollama_base": os.getenv("OLLAMA_BASE_URL", get_setting(conn, "OLLAMA_BASE_URL","http://localhost:11434")),
      "embed_model": os.getenv("OLLAMA_EMBED_MODEL", get_setting(conn, "OLLAMA_EMBED_MODEL","nomic-embed-text:latest")),
    }
    return templates.TemplateResponse("about.html", {"request":request, "stats":stats, "info":info})

def _probe_features():
    conn = get_conn()
    tables = {
      "notes": _db_has(conn, "notes"),
      "tags": _db_has(conn, "tags"),
      "note_embeddings": _db_has(conn, "note_embeddings"),
      "notes_fts": _db_has(conn, "notes_fts", kind="table") or _db_has(conn, "notes_fts", kind="view"),
      "discord_bindings": _db_has(conn, "discord_bindings"),
      "discord_captures": _db_has(conn, "discord_captures"),
      "discord_guild_prefs": _db_has(conn, "discord_guild_prefs"),
      "users": _db_has(conn, "users"),
      "api_tokens": _db_has(conn, "api_tokens"),
    }
    files = {
      "discord_bot": os.path.exists(BASE_DIR/"discord_bot.py"),
      "cli": os.path.exists(BASE_DIR/"sb.py"),
      "makefile": os.path.exists(BASE_DIR/"Makefile"),
      "static_app_js": os.path.exists(BASE_DIR/"static/app.js"),
      "brand_css": os.path.exists(BASE_DIR/"static/brand.css"),
    }
    # endpoints (rough check via source text)
    src = (BASE_DIR/"app.py").read_text(encoding="utf-8", errors="ignore")
    endpoints = {
      "/search": "/search" in src,
      "/export": "/export" in src,
      "/embeddings": "/embeddings" in src,
      "/tags": "/tags" in src,
      "/tokens": "/tokens" in src,
      "/login": "/login" in src,
      "/about": "/about" in src,
      "/compare": "/compare" in src,
    }
    scaffolds = []
    for p in sorted(glob.glob(str(BASE_DIR/"scripts"/"scaffold_*.sh"))):
        try:
            data = open(p,"rb").read()
            h = hashlib.sha256(data).hexdigest()[:12]
            scaffolds.append({"name": os.path.basename(p), "size": len(data), "hash": h})
        except Exception:
            pass
    return {"tables": tables, "files": files, "endpoints": endpoints, "scaffolds": scaffolds}

@app.get("/compare", response_class=HTMLResponse)
def compare(request: Request):
    feat = _probe_features()
    return templates.TemplateResponse("compare.html", {"request": request, **feat})

@app.get("/scaffolds", response_class=HTMLResponse)
def scaffolds_page(request: Request):
    feat = _probe_features()
    return templates.TemplateResponse("scaffolds.html", {"request": request, "scaffolds": feat["scaffolds"]})

@app.post("/theme/apply")
def theme_apply(brand: str = Form(""), mode: str = Form("")):
    conn = get_conn()
    if brand: put_setting(conn, "THEME_BRAND", brand)
    if mode:  put_setting(conn, "THEME_MODE", mode)
