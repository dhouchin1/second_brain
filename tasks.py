...
def process_note(note_id: int):
    ...
    if not tags.strip() and content:
        tags = ollama_generate_tags(content)
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    c.execute(
        "UPDATE notes SET title=?, content=?, summary=?, tags=?, status='complete', timestamp=?, audio_filename=? WHERE id=?",
        (title, content, summary, tags, now, audio_filename, note_id),
    )
    c.execute(
