...
# Purpose: Normalize the scaffolding system:
#   - ensure dirs, .gitignore, hooks
#   - install new.sh / promote.sh helpers if missing
#   - migrate legacy scaffold_*.sh (root or scripts/scaffolds) into inactive/
#   - chmod +x all scaffolds
#   - idempotent
set -euo pipefail
...
migrate_one() {
  ...
  # Extract number if present; otherwise assign next
  if [[ "$base" =~ ([0-9]{3}) ]]; then
    num="${BASH_REMATCH[1]}"
  else
  ...
  # Derive slug from filename without number & 'scaffold' prefix
  slug="$base"
  slug="${slug%.*}"
  ...
  local dest="$INACTIVE_DIR/${num}_${slug}.sh"

  # If already at dest, just ensure exec
  if [[ "$src" == "$dest" ]]; then
    chmod +x "$dest"
    log "ensured exec: $dest"
    return 0
  fi

  # If a different file already exists at dest, pick a new number
...
# --- 4) install helpers if missing: new.sh + promote.sh (drafts in inactive/)
...
NEOF
  chmod +x "$NEW"
  log "installed scripts/scaffolds/new.sh"
else
...
PEOF
  chmod +x "$PROM"
  log "installed scripts/scaffolds/promote.sh"
else
  log "promote.sh already present"
fi

# --- 5) migrate legacy files named like scaffold_037.sh or scaffold-037.sh
migrate_legacy() {
  local p
  for p in \
...
migrate_one() {
  local src="$1"; migrate_one_impl "$src" || true
}
...
# --- 6) chmod +x every scaffold (active + inactive)
