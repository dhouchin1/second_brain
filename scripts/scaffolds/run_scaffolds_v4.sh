...
for f in "${SCRIPTS[@]}"; do
  ...
  # Skip if PR already exists for this branch → BASE
  if gh pr list --base "$BASE" --head "$BR" --json number --jq '.[0].number' | grep -q .; then
    echo "↷ PR already exists for $BR → $BASE, skipping."
    continue
  fi
  ...
  # Ensure log folder exists even after resets
  mkdir -p integration-logs
